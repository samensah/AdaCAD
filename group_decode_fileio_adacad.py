...
def decode(args, batch_input_ids, dec_depth, model, tokenizer):
    ...
    for _i in range(dec_depth):
        if args.model_category == 'causal':
            # Calculate cache_position using the official Transformers approach
            # This is torch.compile-friendly equivalent of torch.arange
            cache_position = torch.ones_like(unit_context_input_ids[0, :], dtype=torch.int64).cumsum(0) - 1
            
            # If we have past_key_values, adjust cache_position for what's already cached
            if past_key_values is not None:
                ...
                # Only process positions that aren't already cached
                if past_length > 0:
            ...
            # cache_position = torch.arange(unit_context_input_ids.shape[1], device=unit_context_input_ids.device)
        ...
        if type(model.generation_config.eos_token_id) == list:
            if real_token_ids_list[0][-1] in model.generation_config.eos_token_id:
                break
        elif real_token_ids_list[0][-1] == model.generation_config.eos_token_id:
            break
...
def parse_args():
    ...
    parser.add_argument(
        "--decode_depth", type=int, default=32, help="",
    )
    parser.add_argument(
